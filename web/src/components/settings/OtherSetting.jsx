...
const OtherSetting = () => {
  ...
  const checkUpdate = async () => {
    try {
      ...
      // Use GitHub releases to check the current Orcas Ai Api version
      // Option 1: Use a public CORS proxy service
      // const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
      // const res = await API.get(
      //   `${proxyUrl}https://api.github.com/repos/OrcasAI/orcas-ai-api/releases/latest`,
      // );

      // Option 2: Use the JSON proxy approach which often works better with GitHub API
      const res = await fetch(
        'https://api.github.com/repos/OrcasAI/orcas-ai-api/releases/latest',
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            // Adding User-Agent which is often required by GitHub API
            'User-Agent': 'orcas-ai-api-update-checker',
          },
        },
  ...
  const openGitHubRelease = () => {
    window.open(
      `https://github.com/OrcasAI/orcas-ai-api/releases/tag/${updateData.tag_name}`,
      '_blank',
    );
  };
